...
Discord slash command로 Codex CLI를 감싸는 개인용 최소 wrapper입니다. `main` bot은 `/srv/codex-discord/prod`에서 실행되고, `staging` bot은 `/srv/codex-discord/dev`에서 실행됩니다. 실제 Codex 작업 대상은 항상 `DEV_WORKSPACE_ROOT` 아래만 허용되므로, 운영 중인 prod checkout을 Codex가 직접 수정하지 않게 막습니다.

현재 이 작업 디렉터리는 실제 git repo/checkouts는 아닙니다. 대신 로컬에서 구조가 보이도록 `./prod`와 `./dev` 디렉터리를 같이 만들어 두었고, 이 둘은 서버의 `/srv/codex-discord/prod`, `/srv/codex-discord/dev`를 흉내 낸 런타임 미러입니다.
...
- `dev/`: staging checkout 형태의 실행 디렉터리
- `prod/`: main checkout 형태의 실행 디렉터리

## 설치

서버에 두 checkout이 있다고 가정합니다.

- `/srv/codex-discord/prod`
- `/srv/codex-discord/dev`
...
BOT_ROLE=staging
CHECKOUT_PATH=/srv/codex-discord/dev
DEV_WORKSPACE_ROOT=/srv/codex-discord/dev
CODEX_DISCORD_CONFIG=/srv/codex-discord/dev/config/config.json
...
DISCORD_ALLOWED_USER_ID=123456789012345678
BOT_ROLE=main
CHECKOUT_PATH=/srv/codex-discord/prod
DEV_WORKSPACE_ROOT=/srv/codex-discord/dev
CODEX_DISCORD_CONFIG=/srv/codex-discord/prod/config/config.json
DISCORD_GUILD_ID=123456789012345678
LOG_LEVEL=INFO
```
...
- `CHECKOUT_PATH`: 현재 봇이 실행 중인 checkout
- `DEV_WORKSPACE_ROOT`: Codex가 수정 가능한 개발 루트. 모든 workspace 매핑은 이 경로 아래여야 함
- `CODEX_DISCORD_CONFIG`: JSON 설정 파일 경로
...
{
  "channels": {
    "111111111111111111": "/srv/codex-discord/dev"
  },
  "timeout_seconds": 900,
  "history_messages": 20,
...
- 모든 `channels` 경로는 `DEV_WORKSPACE_ROOT` 아래여야 합니다.
- 그래서 `main` bot이 prod checkout에서 실행되더라도 `/ask`는 dev workspace만 수정합니다.
...
```bash
sudo cp /srv/codex-discord/prod/systemd/codex-discord-main.service /etc/systemd/system/
sudo cp /srv/codex-discord/dev/systemd/codex-discord-staging.service /etc/systemd/system/

...
- `/new_session`은 부모 채널에서 새 thread를 만들고, 선택한 bot role로 즉시 바인딩
...
- `/ping`: 봇 생존 확인
- `/new_session target_bot_role:... title:...`: 현재 매핑된 채널 아래에 새 session thread 생성
...
3. Discord에서 매핑된 채널에서 `/new_session target_bot_role:staging title:test-session`을 실행합니다.
4. 생성된 thread에서 `/ask prompt:"현재 디렉터리 구조를 설명해줘"`를 실행합니다.

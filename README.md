...
- thread는 첫 thread-scoped 명령을 처리한 bot에 자동 바인딩되며, 바인딩은 thread 이름의 `[bot:staging]` 또는 `[bot:main]` prefix로 유지
...
- main bot 명령: `/ping`, `/new-session`, `/ask`, `/status`, `/diff`
- staging bot 명령: `/ping-staging`, `/new-session-staging`, `/ask-staging`, `/status-staging`, `/diff-staging`
- channel only: `ping`, `ping-staging`, `new-session`, `new-session-staging`
- thread only: `ask`, `ask-staging`, `status`, `status-staging`, `diff`, `diff-staging`
...
3. Discord에서 매핑된 채널에서 `/new-session target_bot_role:staging title:test-session` 또는 `/new-session-staging target_bot_role:staging title:test-session`을 실행합니다.
4. 생성된 thread에서 `/ask-staging prompt:"현재 디렉터리 구조를 설명해줘"`를 실행합니다.

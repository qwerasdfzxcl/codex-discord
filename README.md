...
Discord slash command로 Codex CLI를 감싸는 개인용 최소 wrapper입니다. 현재 구조는 실제 git worktree 기반입니다. `main` bot은 `prod/` checkout에서 실행되고, `staging` bot은 `dev/` checkout에서 실행됩니다. Codex 작업 대상은 항상 컨테이너 루트 아래의 `dev/`만 허용되므로, 운영 중인 prod checkout을 Codex가 직접 수정하지 않게 막습니다.
...
- 그 checkout의 상위 디렉터리 아래 `dev/`를 개발 workspace root로 자동 사용합니다.
...
- `channels`는 절대경로를 써도 되고, 상대경로를 쓰면 자동으로 `dev/` 기준으로 해석됩니다.
- `.`는 `dev/` 자체를 의미합니다.
- 모든 workspace는 최종적으로 `dev/` 아래여야 합니다.
